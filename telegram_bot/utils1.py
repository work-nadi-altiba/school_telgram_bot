...
def create_coloured_certs_wrapper(username , password ,term2=False):
    ...
    def add_averages_to_group_list(grouped_list , skip_art_sport=True):
        for group in grouped_list:
            for item in group:
                if len(item['subjects_assessments_info']) > 0 :
    ...
    create_coloured_certs(students_statistics_assesment_data , term2=term2)
...
def get_student_statistic_info(username,password, identity_nos=None, students_openemis_nos=None, student_ids=None, session=None , teacher_full_name=False):
    ...
    academic_period_id, institution_id , institution_name_code ,modeeriah= class_data['academic_period_id'],class_data['institution_id'],class_data['institution']['code_name'], [i['name'] for i in area_data if i['id'] == class_data['institution']['area_administrative_id']]
...
def process_students_info(students_info_data, identity_types, nationality_data , area_data):
    ...
    for data_item in students_info_data:
        ...
        result['student_id'] = '' if len(data_item['identities']) ==0 else data_item['identities'][0]['number']
...
def create_tables(auth , grouped_list ,term2=False ,template='./templet_files/tamplete_table.xlsx.xlsx'  , outdir='./send_folder/'):
    # auth = get_auth(username , password)
    ...
    for group in grouped_list:
        ...
        for row_number, dataFrame in enumerate(sort_dictionary_list_based_on(group ,key='student__full_name',simple=False,reverse=False), start=4):
...
def create_coloured_certs(grouped_list , term2=False ,template='./templet_files/نموذج شهادات ملونة.xlsx' , outdir='./send_folder/'):
    ...
    for group in assessments_data_groups:
        ...
        for group_item in group:
            ...
            student_statistic_info = [i for i in statistic_data['students_info'] if i['student_id'] == wanted_id ]  
            ...
            #المدرسة و رقمها الوطني
            sheet2[c['schoo_name']]= group_item['student_school_name']
            ...
            # اللواء
            sheet2[c['bridge']]= grouped_list['school_bridge']
            # الديانة
            sheet2[c['religion']]= statistic_data['religion']
            # عنوان الطالب
            sheet2[c['bridge']]= statistic_data['address']
            # عنوان المدرسة
            sheet2[c['bridge']]= grouped_list['school_address']
            # العام الدراسي الاول 
            sheet2[c['bridge']]= grouped_list['academic_year_1']
            # العام الدراسي الثاني 
            sheet2[c['bridge']]= grouped_list['academic_year_2']
            # الاسم على الوجه الثاني
            sheet2[c['student_name2']] = group_item['student__full_name']
            # الصف 
            sheet2[c['bridge']]= group_item['student_grade_name'].replace('الصف' ,'')
            # غياب الفصل الاول 
            ...
            if 'ثامن' in group_item['student_grade_name'] or 'تاسع' in group_item['student_grade_name'] or 'عاشر' in group_item['student_grade_name']:
                ...
                sheet2[c['islamic_education']] = islam_subject_maxMark /2
                sheet2['F'+c['islamic_education'][1:]] = islam_subject_maxMark
            sheet2['H'+c['islamic_education'][1:]] = islam_subject[0][value_item] if islam_subject and len(islam_subject[0]) != 0 else ''
            sheet2['I'+c['islamic_education'][1:]] = islam_subject[0][1] if term2 and islam_subject and len(islam_subject[0]) != 0 else ''
            sheet2['K'+c['islamic_education'][1:]] = (islam_subject[0][value_item] + islam_subject[0][1])/2 if term2 and islam_subject and len(islam_subject[0]) != 0 else ''
            # if term2:
            #     sheet2['G'+c['islamic_education']] = convert_avarage_to_words((islam_subject[0][value_item] + islam_subject[0][1])/2) if islam_subject else ''
            #     sheet2['J'+c['islamic_education']] = score_in_words(((islam_subject[0][value_item] + islam_subject[0][1])/2),max_mark=maxMark) if islam_subject else ''
            # else:
            #     sheet2['G'+c['islamic_education']] = convert_avarage_to_words(islam_subject[0][value_item]) if islam_subject else ''
            #     sheet2['J'+c['islamic_education']] = score_in_words(islam_subject[0][value_item],max_mark=maxMark) if islam_subject else ''

            # اللغة العربية
            arabic_subject = [value for key ,value in group_item['subject_sums'].items() if 'عربية' in key]
            if 'ثامن' in group_item['student_grade_name'] or 'تاسع' in group_item['student_grade_name'] or 'عاشر' in group_item['student_grade_name']:
                arabic_subject_maxMark = 300
                sheet2['F'+c['arabic_language'][1:]] = arabic_subject_maxMark / 2
                sheet2[c['arabic_language']] = arabic_subject_maxMark
            else:
                arabic_subject_maxMark = 100
                sheet2['F'+c['arabic_language'][1:]] = arabic_subject_maxMark / 2
                sheet2[c['arabic_language']] = arabic_subject_maxMark
            sheet2['H' + c['arabic_language'][1:]] = arabic_subject[0][value_item] if arabic_subject and len(arabic_subject[0]) != 0 else ''
            sheet2['I' + c['arabic_language'][1:]] = arabic_subject[0][1] if term2 and arabic_subject and len(arabic_subject[0]) != 0 else ''
            sheet2['K' + c['arabic_language'][1:]] = (arabic_subject[0][value_item] + arabic_subject[0][1])/2 if term2 and arabic_subject and len(arabic_subject[0]) != 0 else ''
            # if term2:
            #     sheet2['J19'] = score_in_words(((arabic_subject[0][value_item] + arabic_subject[0][1])/2),max_mark=maxMark) if arabic_subject else ''
            #     sheet2['F'+] = maxMark / 2
            #     sheet2[] = convert_avarage_to_words((arabic_subject[0][value_item] + arabic_subject[0][1])/2) if arabic_subject else maxMark
            # else:
            #     sheet2['J19'] = score_in_words(arabic_subject[0][value_item],max_mark=maxMark) if arabic_subject else ''
            #     sheet2['F'+] = maxMark / 2
            #     sheet2[] = convert_avarage_to_words(arabic_subject[0][value_item]) if arabic_subject else maxMark

            # اللغة الانجليزية 
            english_subject = [value for key ,value in group_item['subject_sums'].items() if 'جليزية' in key]
            if 'ثامن' in group_item['student_grade_name'] or 'تاسع' in group_item['student_grade_name'] or 'عاشر' in group_item['student_grade_name']:
                english_subject_maxMark = 200
                sheet2['F'+c['english_language'][1:]] = english_subject_maxMark / 2
                sheet2[c['english_language']] = english_subject_maxMark
            else:
                english_subject_maxMark = 100
                sheet2['F'+c['english_language'][1:]] = english_subject_maxMark / 2
                sheet2[c['']] = maxMark
            sheet2['H'+c['english_language'][1:]] = english_subject[0][value_item] if english_subject and len(english_subject[0]) != 0 else ''
            sheet2['I'+c['english_language'][1:]] = english_subject[0][1] if term2 and english_subject and len(english_subject[0]) != 0 else ''
            sheet2['K'+c['english_language'][1:]] = (english_subject[0][value_item] + english_subject[0][1])/2 if term2 and english_subject and len(english_subject[0]) != 0 else ''
            # if term2:
            #     sheet2['J20'] = score_in_words(((english_subject[0][value_item] + english_subject[0][1])/2),max_mark=maxMark) if english_subject else ''
            #     sheet2['F'+c[''][1:]] = maxMark / 2
            #     sheet2[] = convert_avarage_to_words((english_subject[0][value_item] + english_subject[0][1])/2) if english_subject else maxMark
            # else:
            #     sheet2['J20'] = score_in_words(english_subject[0][value_item],max_mark=maxMark) if english_subject else ''
            #     sheet2['F'+c[''][1:]] = maxMark / 2
            #     sheet2[] = convert_avarage_to_words(english_subject[0][value_item]) if english_subject else maxMark

            # الرياضيات 
            math_subject = [value for key ,value in group_item['subject_sums'].items() if 'رياضيات' in key]
            if 'ثامن' in group_item['student_grade_name'] or 'تاسع' in group_item['student_grade_name'] or 'عاشر' in group_item['student_grade_name']:
                math_subject_maxMark = 200
                sheet2['F'+c['mathematics'][1:]] = math_subject_maxMark / 2
                sheet2[c['mathematics']] = math_subject_maxMark
            else:
                math_subject_maxMark = 100
                sheet2['F'+c['mathematics'][1:]] = math_subject_maxMark / 2
                sheet2[c['mathematics']] = math_subject_maxMark
            sheet2['H'+c['mathematics'][1:]] = math_subject[0][value_item] if math_subject and len(math_subject[0]) != 0 else ''
            sheet2['I'+c['mathematics'][1:]] = math_subject[0][1] if term2 and math_subject and len(math_subject[0]) != 0 else ''
            sheet2['K'+c['mathematics'][1:]] = (math_subject[0][value_item] + math_subject[0][1])/2 if term2 and math_subject and len(math_subject[0]) != 0 else ''
            # if term2:
            #     sheet2['J21'] = score_in_words(((math_subject[0][value_item] + math_subject[0][1])/2),max_mark=maxMark) if math_subject else ''
            #     sheet2['F'+c[''][1:]] = maxMark / 2
            #     sheet2[] = convert_avarage_to_words((math_subject[0][value_item] + math_subject[0][1])/2) if math_subject else maxMark
            # else:
            #     sheet2['J21'] = score_in_words(math_subject[0][value_item],max_mark=maxMark) if math_subject else ''
            #     sheet2['F'+c[''][1:]] = maxMark / 2
            #     sheet2[] = convert_avarage_to_words(math_subject[0][value_item]) if math_subject else maxMark

            # التربية الاجتماعية و الوطنية 
            social_subjects = [value for key ,value in group_item['subject_sums'].items() if 'اجتماعية و الوطنية' in key]
            if 'ثامن' in group_item['student_grade_name'] or 'تاسع' in group_item['student_grade_name'] or 'عاشر' in group_item['student_grade_name']:
                social_subjects_maxMark = 200
                sheet2['F'+c['social_studies'][1:]] = social_subjects_maxMark / 2
                sheet2[c['social_studies']] = social_subjects_maxMark
                sheet2['D22'] = int(social_subjects[0][value_item]*(2/3)) if social_subjects and len(social_subjects[0]) != 0 else ''
            elif 'سادس' in group_item['student_grade_name'] or 'سابع' in group_item['student_grade_name']:
                sheet2['D22'] = int(social_subjects[0][value_item]/3) if social_subjects and len(social_subjects[0]) != 0 else ''
            ...
            # sheet'H'+22'] = social_subjects[0][value_item] if social_subjects and len(social_subjects[0]) != 0 else ''
            sheet2['I'+c[''][1:]] = social_subjects[0][1] if term2 and social_subjects and len(social_subjects[0]) != 0 else ''
            sheet2['K'+c[''][1:]] = (social_subjects[0][value_item] + social_subjects[0][1])/2 if term2 and social_subjects and len(social_subjects[0]) != 0 else ''
            # if term2:
            #     sheet2['J22'] = score_in_words(int(((social_subjects[0][value_item] + social_subjects[0][1])/2)*(2/3)),max_mark=maxMark) if social_subjects else ''
            #     sheet2['F'+c[''][1:]] = maxMark / 2
            #     sheet2[] = convert_avarage_to_words((social_subjects[0][value_item] + social_subjects[0][1])/2) if social_subjects else maxMark
            # else:
            #     sheet2['J22'] = score_in_words(int(social_subjects[0][value_item]*(2/3)),max_mark=maxMark) if social_subjects else ''
            #     sheet2['F'+c[''][1:]] = maxMark / 2
            #     sheet2[] = convert_avarage_to_words(social_subjects[0][value_item]) if social_subjects else maxMark

            # العلوم
            science_subjects = [value for key ,value in group_item['subject_sums'].items() if 'العلوم' in key]
            if 'ثامن' in group_item['student_grade_name'] :
                science_subjects_maxMark = 200
                sheet2['F'+c['science'][1:]] = science_subjects_maxMark / 2
                sheet2[c['science']] = science_subjects_maxMark
            elif 'تاسع' in group_item['student_grade_name'] or 'عاشر' in group_item['student_grade_name']:
                ...
                sheet2['F'+c['science'][1:]] = science_subjects_maxMark / 2
                sheet2[c['science']] = science_subjects_maxMark
            sheet2['H'+c['science'][1:]] = science_subjects[0][value_item] if science_subjects and len(science_subjects[0]) != 0 else ''
            sheet2['I'+c['science'][1:]] = science_subjects[0][1] if term2 and science_subjects and len(science_subjects[0]) != 0 else ''
            sheet2['K'+c['science'][1:]] = (science_subjects[0][value_item] + science_subjects[0][1])/2 if term2 and science_subjects and len(science_subjects[0]) != 0 else ''
            # if term2:
            #     sheet2['J23'] = score_in_words( ((science_subjects[0][value_item] + science_subjects[0][1])/2),max_mark=maxMark) if  science_subjects else ''
            #     sheet2['F'+c[''][1:]] = maxMark / 2
            #     sheet2[] = convert_avarage_to_words((science_subjects[0][value_item] + science_subjects[0][1])/2) if science_subjects else maxMark
            # else:
            #     sheet2['J23'] = score_in_words( science_subjects[0][value_item],max_mark=maxMark) if  science_subjects else ''
            #     sheet2['F'+c[''][1:]] = maxMark / 2
            #     sheet2[] = convert_avarage_to_words(science_subjects[0][value_item]) if science_subjects else maxMark
            ...
            sheet2['H'+c['visual_arts'][1:]] = art_subject[0][value_item] if art_subject and len(art_subject[0]) != 0 else ''
            sheet2['I'+c['visual_arts'][1:]] = art_subject[0][1] if term2 and art_subject and len(art_subject[0]) != 0 else ''
            sheet2['K'+c['visual_arts'][1:]] = (art_subject[0][value_item] + art_subject[0][1])/2 if term2 and art_subject and len(art_subject[0]) != 0 else ''
            # if term2:
            #     sheet2['G24'] = convert_avarage_to_words((art_subject[0][value_item] + art_subject[0][1])/2) if art_subject else ''
            #     sheet2['J24'] = score_in_words(((art_subject[0][value_item] + art_subject[0][1])/2) ) if art_subject else ''
            # else:
            ...
            sheet2['H'+c['physical_education'][1:]] = sport_subject[0][value_item] if sport_subject and len(sport_subject[0]) != 0 else ''
            sheet2['I'+c['physical_education'][1:]] = sport_subject[0][1] if term2 and sport_subject and len(sport_subject[0]) != 0 else ''
            sheet2['K'+c['physical_education'][1:]] = (sport_subject[0][value_item] + sport_subject[0][1])/2 if term2 and sport_subject and len(sport_subject[0]) != 0 else ''
            # if term2:
            #     sheet2['G25'] = convert_avarage_to_words((sport_subject[0][value_item] + sport_subject[0][1])/2) if sport_subject else ''
            #     sheet2['J25'] = score_in_words(((sport_subject[0][value_item] + sport_subject[0][1])/2) ) if sport_subject else ''
            # else:
            ...
            sheet2['H'+c['vocational_education'][1:]] = vocational_subject[0][value_item] if vocational_subject and len(vocational_subject[0]) != 0 else ''
            ...
            sheet2['K'+c['vocational_education'][1:]] = (vocational_subject[0][value_item] + vocational_subject[0][1])/2 if term2 and vocational_subject and len(vocational_subject[0]) != 0 else ''
            # if term2:
            #     sheet2['G26'] = convert_avarage_to_words((vocational_subject[0][value_item] + vocational_subject[0][1])/2) if vocational_subject else ''
            #     sheet2['J26'] = score_in_words(((vocational_subject[0][value_item] + vocational_subject[0][1])/2) ) if vocational_subject else ''
            # else:
            ...
            sheet2['H'+c['computer'][1:]] = computer_subject[0][value_item] if computer_subject and len(computer_subject[0]) != 0 else ''
            sheet2['I'+c['computer'][1:]] = computer_subject[0][1] if term2 and computer_subject and len(computer_subject[0]) != 0 else ''
            sheet2['K'+c['computer'][1:]] = (computer_subject[0][value_item] + computer_subject[0][1])/2 if term2 and computer_subject and len(computer_subject[0]) != 0 else ''
            # if term2:
            #     sheet2['G27'] = convert_avarage_to_words((computer_subject[0][value_item] + computer_subject[0][1])/2) if computer_subject else ''
            #     sheet2['J27'] = score_in_words(((computer_subject[0][value_item] + computer_subject[0][1])/2) ) if computer_subject else ''
            # else:
            ...
            sheet2['H'+c['financial_culture'][1:]] = financial_subject[0][value_item] if financial_subject and len(financial_subject[0]) != 0 else ''
            sheet2['I'+c['financial_culture'][1:]] = financial_subject[0][1] if term2 and financial_subject and len(financial_subject[0]) != 0 else ''
            sheet2['K'+c['financial_culture'][1:]] = (financial_subject[0][value_item] + financial_subject[0][1])/2 if term2 and financial_subject and len(financial_subject[0]) != 0 else ''
            # if term2:
            #     sheet2['G28'] = convert_avarage_to_words((financial_subject[0][value_item] + financial_subject[0][1])/2) if financial_subject else ''
            #     sheet2['J28'] = score_in_words(((financial_subject[0][value_item] + financial_subject[0][1])/2) ) if financial_subject else ''
            # else:
            ...
            sheet2['H'+c['french_language'][1:]] = franch_subject[0][value_item] if franch_subject and len(franch_subject[0]) != 0 else ''
            sheet2['I'+c['french_language'][1:]] = franch_subject[0][1] if term2 and franch_subject and len(franch_subject[0]) != 0 else ''
            sheet2['K'+c['french_language'][1:]] = (franch_subject[0][value_item] + franch_subject[0][1])/2 if term2 and franch_subject and len(franch_subject[0]) != 0 else ''
            # if term2:
            #     sheet2['G29'] = convert_avarage_to_words((franch_subject[0][value_item] + franch_subject[0][1])/2) if franch_subject else ''
            #     sheet2['J29'] = score_in_words(((franch_subject[0][value_item] + franch_subject[0][1])/2) ) if franch_subject else ''
            # else:
            ...
            sheet2['H'+c['christian_religion'][1:]] = christian_subject[0][value_item] if christian_subject and len(christian_subject[0]) != 0 else ''
            sheet2['I'+c['christian_religion'][1:]] = christian_subject[0][1] if term2 and christian_subject and len(christian_subject[0]) != 0 else ''
            sheet2['K'+c['christian_religion'][1:]] = (christian_subject[0][value_item] + christian_subject[0][1])/2 if term2 and christian_subject and len(christian_subject[0]) != 0 else ''
            # if term2:
            #     sheet2['G30'] = convert_avarage_to_words((christian_subject[0][value_item] + christian_subject[0][1])/2) if christian_subject else ''
            #     sheet2['J30'] = score_in_words(((christian_subject[0][value_item] + christian_subject[0][1])/2) ) if christian_subject else ''
            # else:
            ...
            if term2 :
                ...
                #المعدل المئوي بالرقام 
                sheet2[c['average']]= group_item['t1+t2+year_avarage'][0]
                # #بالحروف
                # sheet2['e32']= convert_avarage_to_words(group_item['t1+t2+year_avarage'][0]) if group_item else ''
                # #ترتيب الطالب على الصف 
                # sheet2['j32']= counter-1
            ...
            # sheet2['b36']= ''
            #اسم و توقيع مدير المدرسة
            sheet2[c['principal_name']]= grouped_list['principal_name'] 
        ...
        dic_list.append(
                    {
                        ...
                        'student_nat_id':i['user']['identity_number'],
...
def make_request(url, auth ,session=None,timeout_seconds=10):
